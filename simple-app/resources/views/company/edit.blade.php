<x-layout>
    ...
    <form action="/company/{{ $company->id }}" method="POST">
                        ...
                        <div class="mt-2">
                            <div
                                ...
                                <input type="text" name="name" id="name"
                                    ...
                                    placeholder="Ice Cream Company" value="{{ $company->name }}">
                        ...
                        <div class="mt-2">
                            <textarea name="description" id="description" rows="3"
                                class="block w-full rounded-md bg-white px-3 py-1.5 text-base outline-1 -outline-offset-1 outline-gray-300 text-gray-700 
                                    placeholder:text-gray-500  focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">{{ $company->description }}</textarea>
                        </div>
                        @error('description')
                            <p class="ml-1 pt-1 text-red-500 ">{{ $message }}</p>
                        @enderror
                        ...
                        <div class="mt-2">
                            <div
                                ...
                                <input type="text" name="address" id="address"
                                    class="block min-w-0 grow py-1.5 pr-3 pl-1 text-base text-gray-700 
                                    placeholder:text-gray-400 focus:outline-none sm:text-sm/6"
                                    placeholder="Bio,Tagudin Ilocos Sur" value="{{ $company->address }}">
                            </div>
                        </div>
                        @error('address')
                            <p class="ml-1 pt-1 text-red-500 ">{{ $message }}</p>
        ...
        <div class="mt-6 flex items-center justify-end gap-x-6">
            <a href="/company" class="text-sm/6 font-semibold text-white">Cancel</a>
            <button type="submit"
